...
import { renderHotelAdminInvite } from '../src/render';
import {
  HOTEL_ADMIN_INVITE_SUBJECT,
  renderHotelAdminInviteHtml,
} from '../src/templates/hotel-admin-invite.html';
import { renderHotelAdminInviteText } from '../src/templates/hotel-admin-invite.text';
...
describe('renderHotelAdminInvite', () => {
  it('returns subject, html, and text for the given input', () => {
    const result = renderHotelAdminInvite({
      link: FIXTURE_LINK,
      expiresInMinutes: FIXTURE_EXPIRY,
      hotelName: FIXTURE_HOTEL_NAME,
    });
    expect(result.subject).toBe(HOTEL_ADMIN_INVITE_SUBJECT);
...
describe('renderHotelAdminInviteHtml', () => {
  it('escapes HTML special characters in the link to prevent injection', () => {
    ...
    const html = renderHotelAdminInviteHtml({
      link: malicious,
      expiresInMinutes: FIXTURE_EXPIRY,
      hotelName: FIXTURE_HOTEL_NAME,
    });
...
describe('renderHotelAdminInviteText', () => {
  it('contains the link on its own line for client auto-linkify', () => {
    const text = renderHotelAdminInviteText({
      link: FIXTURE_LINK,
      expiresInMinutes: FIXTURE_EXPIRY,
      hotelName: FIXTURE_HOTEL_NAME,
    });
