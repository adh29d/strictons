...
// itself is a File/Blob and can't be zod-validated — it goes through
// parseCandidatesCsv. This schema covers the only zod-validatable part
// of the FormData: the hotel id. Completes the one-schema-per-action
// pattern (§2's original inventory missed it because the CSV file-
// handling details were nailed down later — added in commit 7).
...
| Insert shape | `{ hotel_id, source: 'google_places', google_place_id: placeId, name, address, category, distance_m, phone, website, proposed_by: staffUserId, status: 'proposed' }`. `name` / `address` / `phone` / `website` are populated from the Place Details response; `category` is `input.category ?? placeDetails.primaryType ?? null` (override wins, else derive from `primaryType`, else null). `distance_m` is **always `null`** for a Google Places add — `AddFromGooglePlacesInputSchema` carries no distance input, the v1 Places response has no distance field, and `hotels` has no location column to measure from. The column is in the insert shape for parity with the manual-add insert; it just can't be populated here in Phase 6. |
| Returns success | `{ ok: true, message: 'Candidate added.', candidateId }` |
| Returns failure | Same shape as manual; additional `reason` values `place_not_found` (`PlacesUpstreamError` with `status === 404`), `places_api_failed` (`PlacesUpstreamError` any other status / timeout / network), `missing_api_key` (`PlacesConfigError` — no `GOOGLE_PLACES_API_KEY`), `duplicate_place` (Postgres 23505 from the partial unique index — surfaces as `fieldErrors.placeId` pointing at the existing row). |
...
| Request body | `{ query: string, hotelId: string }` validated with `GooglePlacesSearchInputSchema`. The `hotelId` is validated for shape but the Route Handler does not use or record it (searches are not audited — see the Audit row); it rides in the request contract so the client search panel can carry its hotel context without a separate field. |
| External call | Google Places Text Search via the adapter in §5. |
| Response success | `{ ok: true, results: Array<{ placeId, name, formattedAddress, primaryType?, location? }> }`. Capped at top 10 results (the adapter caps internally; the Route Handler re-slices defensively so the §3.2 response contract holds at the API boundary regardless of adapter changes). |
| Response failure | `{ ok: false, error: string }` with HTTP 400 (validation / unparseable body), 401 (unauth), 429 (rate-limit), 500 (missing/invalid `GOOGLE_PLACES_API_KEY` configuration — `PlacesConfigError`; server-side misconfiguration, not a client error or an upstream failure), 502 (upstream Google failure — `PlacesUpstreamError`) status codes. |
...
### 7.3a Parser return contract

`parseCandidatesCsv(content: string)` returns a discriminated union:

- **`{ ok: true; rows: CsvRow[]; rejected: CsvRejection[] }`** — the file parsed structurally fine. `rows` are the per-row-validated payloads; `rejected` is `{ rowNumber, error }[]` for rows that failed `CsvRowSchema`. Either list may be empty (an all-valid file has `rejected: []`; an all-invalid-but-structurally-fine file has `rows: []` — the caller treats that as success-with-N=0, see §3.1).
- **`{ ok: false; error: string; reason: CsvParseFailReason }`** — a fatal condition; the parser refuses to return partial results. `error` is the user-facing message; `reason` is the machine-readable discriminant: `'oversized' | 'empty' | 'missing_name_column' | 'no_data_rows' | 'too_many_rows' | 'parse_failed'`.

The `reason` discriminant was added in commit 7 — the commit-4 parser shipped with `{ ok: false; error }` and "no caller wired yet"; commit 7 is the first caller and needs a machine-readable signal so `uploadCandidateCsv` maps fatal cases to frozen §8 audit reasons **without string-matching the user-facing `error`** (a brittle coupling — a future copy-edit to an error message would silently break the audit mapping). `'parse_failed'` is emitted only if `Papa.parse` itself throws (a third-party-library boundary defensively wrapped; `Papa.parse` on a string doesn't throw for malformed CSV). The action's `reason` → audit-reason map: `oversized`/`too_many_rows`/`missing_name_column` map 1:1; `empty`/`no_data_rows`/`parse_failed` all collapse to the audit reason `parse_failed`.
...
| `candidate_add_failed`                        | `strictons_staff` \| `hotel_admin` | Failed add. `after.reason` ∈ `{validation_failed, hotel_not_found, place_not_found, places_api_failed, missing_api_key, duplicate_place, insert_failed, list_not_editable}` — `missing_api_key` added in the commit-6 plan-review round (the `PlacesConfigError` path from §6.6). The staff-side Google Places add uses the subset without `list_not_editable` (§3.1 gives it no list-state precondition; `list_not_editable` is the hotel-side member). |
| `candidate_csv_imported`                      | `strictons_staff`                  | Successful CSV import. `after = { imported, rejected }` counts.                                                                                                                                                                                                                                                                                                                                                                                          |
| `candidate_csv_import_failed`                 | `strictons_staff`                  | Failed CSV import. `after.reason` ∈ `{validation_failed, hotel_not_found, oversized, too_many_rows, missing_name_column, parse_failed, insert_failed}` — `hotel_not_found` added in the commit-7 plan-review round (the SELECT-then-act pattern SELECTs the hotel first; analogous to the commit-5 extension of the list-state events).                                                                                                                  |
...
| `candidate_list_approve_failed`               | `hotel_admin`                      | Failure. `after.reason` ∈ `{validation_failed, hotel_not_found, wrong_state, update_failed}`                                                                                                                                                                                                                                                                                                                                                             |
...
Reason-set extensions (commit 6): `missing_api_key` added to `candidate_add_failed` (the `PlacesConfigError` path). Reason-set extensions (commit 7): `hotel_not_found` added to `candidate_csv_import_failed` — `uploadCandidateCsv` SELECTs the hotel before parsing (the SELECT-then-act pattern), and a non-existent hotelId needs its own audit reason rather than overloading `validation_failed` (which is for a malformed FormData shape, a structurally different failure).

Reason-set extensions (commit 9): `validation_failed` and `hotel_not_found` added to `candidate_list_approve_failed`, mirroring the commit-5 extension of the symmetric `candidate_list_mark_ready_for_review_failed` event. Same rationale: a tampered/malformed `hotelId` (validation_failed) and a SELECT-then-act miss (hotel_not_found) are structurally distinct from a genuine wrong-state transition, and overloading `wrong_state` to absorb them would degrade operational triage. `list_not_editable` is the only reason on `candidate_add_failed` / `candidate_remove_failed` that the commit-9 hotel-side actions emit (the staff-side actions never emit it — §3.1 gives them no list-state precondition; it was added to both sets in earlier rounds in anticipation of the hotel-side actions landing here). RLS does not gate on `hotels.approval_state` for the hotel-admin INSERT/UPDATE policies on `candidate_businesses`, so the action-layer `list_not_editable` rejection is the only enforcement point — defense in depth, with RLS permissive of any state and the action narrowing to `{candidate_list_with_hotel, paused_awaiting_hotel_response}`.
...
- `apps/admin/lib/parse-candidates-csv.test.ts` — well-formed CSV; missing optional columns; missing required `name` column (fatal); quoted-comma fields; BOM-prefixed file; extra columns ignored; header normalisation (capitalised / uppercase / whitespace-padded / mixed-case `name`, plus `Contact_Email` → `contact_email`); per-row validation failure mix with spreadsheet rowNumbers; mid-file blank line preserves rowNumber alignment; trailing newline produces no spurious rejection; >500 rows (fatal) and exactly-500 accepted; >1 MiB (fatal); empty file (fatal); header-only file (fatal — distinct `"no data rows"` message). Each fatal case also asserts the `reason` discriminant; a `Papa.parse`-throws case (via a scoped `vi.spyOn`) asserts `reason: 'parse_failed'`.
- `apps/admin/app/(protected)/hotels/[id]/candidates/actions.test.ts` — every action, every branch (per Phase 5 pattern). `revalidatePath` mocked. `createServiceRoleClient` mocked. **Explicitly covers the `addCandidateFromGooglePlaces` Postgres 23505 → `duplicate_place` error-mapping branch** — i.e. when the partial unique index rejects the INSERT, the action returns `{ ok: false, fieldErrors: { placeId: '<message pointing at existing row>' } }` and audits `candidate_add_failed` with `reason: 'duplicate_place'`. (The constraint itself is tested at pgTAP level; the action's error-mapping is tested here.) Also covers the staff-removal status=`removed_by_strictons` write (Q3) and the reopen `reason` payload presence/absence (Q4). For `uploadCandidateCsv`: `parseCandidatesCsv` is mocked at the `apps/admin/lib` boundary (the parser is its own tested boundary; papaparse is never mocked here); cases cover happy / mixed / all-rejection-N=0 (success, INSERT skipped) / not-staff / `validation_failed` (bad hotelId + missing/non-File file) / `hotel_not_found` / each parser fatal `reason` → audit reason / `insert_failed` (parser rejections still surface).
- `apps/admin/app/api/places/search/route.test.ts` — auth gate (401 when not staff, and the rate-limit bucket is NOT consumed), unparseable body (400), zod validation failure (400), rate-limit overflow (429 + `Retry-After` header, upstream NOT called), upstream success (200, top-10 cap enforced at the route boundary), `PlacesUpstreamError` (502), `PlacesConfigError` / missing key (500). The adapter functions (`searchPlacesText`, `consumeRateLimit`) are mocked — the adapter is its own tested boundary (commit 3); `fetch` is never mocked here.
