/**
 * Sentry Node.js (server) SDK initialization for the admin app.
 *
 * Loaded by `instrumentation.ts` via the Next 15 `register()` hook
 * when the runtime is 'nodejs'. SDK + DSN only this phase.
 *
 ...
 * see Phase 4 commit 2's diagnostic for the root cause. Without the
 * wrapper, Vercel's serverless function freeze drops unflushed
 * events, AND Next.js production-mode error sanitisation replaces
 * the original error message with a generic placeholder before
 * onRequestError sees it.
 */
